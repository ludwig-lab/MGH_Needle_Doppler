...
info = imfinfo(fullfile(data_files_folder, oa_avg_file));
oa_avg = imread(fullfile(data_files_folder, oa_avg_file), info=info);
rr_avg = imread(fullfile(data_files_folder, rr_avg_file));
ii_avg = imread(fullfile(data_files_folder, ii_avg_file));
oa_avg = oa_avg(1:1024,:,:);
rr_avg = rr_avg(1:1024,:);
ii_avg = ii_avg(1:1024,:);
...
%
%
% writerObj = VideoWriter('salmon_rr2.avi','Uncompressed AVI');
...
num_rows_ii_file = 2048

for i=1:700

    fseek(ii_file_id,1024*1024 + num_rows_ii_file*1024*(i-1+200),'bof');
    img = fread(ii_file_id,[num_rows_ii_file 1024],'uint8=>single').';
    ...
    oa_sInt(1400:1410,:,:) = 0; % commented out by Rex until we have oa data ready
